...
class SpriteRenderer():
    ...
    def events(self):
        ...
        if pygame.key.get_pressed()[pygame.K_SPACE]:
            ...
            if self.dt != self.startDt -self.meter.getSlowDownAmount() and not self.meter.getEmpty():
    ...
    def resetPeopleClicks(self):
        ...
        currentLayerPeople = self.getGridLayer("layer " + str(self.currentLayer)).getPeople()
        for person in totalPeople:
            if person in currentLayerPeople or self.currentLayer == 4: # Always went every person to be clickable on the top layer
